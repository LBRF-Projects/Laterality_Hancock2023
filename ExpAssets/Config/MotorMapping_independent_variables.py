...
>> MotorMapping_ind_vars.add_variable("color", str)
>> MotorMapping_ind_vars.add_variable("size", float)
>> MotorMapping_ind_vars.add_variable("active", bool)
>> MotorMapping_ind_vars.add_variable("count", int)

Finally, we add values to each variable. This can be done one at a time, as in the colors example below:

>> MotorMapping_ind_vars['color'].add_value("blue")
>> MotorMapping_ind_vars['color'].add_value("blue")
>> MotorMapping_ind_vars['color'].add_value("blue")

Or altogether in a comma-separated set, as in the 'count' example:

>> MotorMapping_ind_vars['count'].add_values(1,2,3,4,5)
...
>> MotorMapping_ind_vars['size'].add_value(1.0, 2)
>> MotorMapping_ind_vars['size'].add_value(2.0, 2)
>> MotorMapping_ind_vars['size'].add_value(5.0)
...
>> MotorMapping_ind_vars['active'].add_values((True, 5), False)

which is equivalent to:

>> MotorMapping_ind_vars['active'].add_value(True, 5)
>> MotorMapping_ind_vars['active'].add_value(False)
...
>> MotorMapping_ind_vars['size'].enabled = False
...
>> MotorMapping_ind_vars['size'][1.0].enabled = False
